...
export const firecrawlApi = {
  ...
  // Trigger URL verification worker
  async verifyUrls(params: {
    source_code?: string;
    all?: boolean;
    id?: string;
    recheck_valid?: boolean;
    recheck_days?: number;
  }): Promise<FirecrawlResponse<{
    stats: { total: number; valid: number; auto_fixed: number; needs_review: number; invalid: number };
    results: Array<{
      id: string;
      source_code: string;
      identifier: string;
    ...
    const { data, error } = await supabase.functions.invoke('url-verify-worker', {
      body: params,
    });
