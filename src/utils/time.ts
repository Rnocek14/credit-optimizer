...
export function parseTimeEstimateToMinutes(input?: string | null): number | null {
  ...
  // Handle complex hour-minute formats like "1h 15m", "2hr 30min", "1 hour and 15 minutes"
  const complexHourMin = s.match(/^(\d+(?:[,.]\d+)?)\s*(?:h|hr|hour|hours?)\s*(?:and\s+)?(\d+(?:[,.]\d+)?)\s*(?:m|min|mins|minutes?)$/);
  if (complexHourMin) {
    const hours = parseFloat(complexHourMin[1].replace(',', '.'));
    const minutes = parseFloat(complexHourMin[2].replace(',', '.'));
  ...
  // "0.5hr", "1hr", "2hrs", "1hour", "2hours", "1,5hr" (European decimal)
