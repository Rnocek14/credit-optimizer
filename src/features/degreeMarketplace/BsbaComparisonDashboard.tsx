...
interface BsbaComparisonDashboardProps {
  institutionCode: InstitutionCode;
  institutionName: string;
}

export function BsbaComparisonDashboard({
  institutionCode,
  institutionName,
}: BsbaComparisonDashboardProps) {
  ...
  if (isLoading) {
    return (
      <div className="space-y-2 rounded-xl border bg-card p-4">
        <h2 className="text-lg font-semibold">{institutionName} BSBA Comparison</h2>
    ...
    return (
      <div className="space-y-2 rounded-xl border bg-card p-4">
        ...
        <p className="text-sm text-muted-foreground">
          No optimized templates available yet. Seed at least a Standard and Alt-Credit Max
          template.
        </p>
      </div>
    );
  ...
  const standard = optimizedTemplates.find((t) => t.template.track_type === 'standard');
  const altMax = optimizedTemplates.find((t) => t.template.track_type === 'alt_max');
  ...
  const altPctStandard =
    s.totalCredits > 0 ? Math.round((s.totalAltCredits / s.totalCredits) * 100) : 0;
  const altPctAlt =
    a.totalCredits > 0 ? Math.round((a.totalAltCredits / a.totalCredits) * 100) : 0;

  return (
    <div className="rounded-xl border bg-card p-4">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold">{institutionName} BSBA – Path Comparison</h2>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <ComparisonCard
          label="Standard"
          subtitle={`${institutionName} courses first`}
          metrics={s}
      ...
      <p className="mt-3 text-[11px] text-muted-foreground md:text-xs">
        Estimates based on placeholder cost-per-credit values. Always verify with {institutionName}
        's official tuition and partner pages before advising students.
      </p>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 grid grid-cols-2 gap-3 text-xs md:text-sm">
        ...
        />
        <Metric
          label={`${institutionName} Residency`}
