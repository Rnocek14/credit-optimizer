...
export function useApplyDimmingV2({ nodes, edges, useClassSystem = false }: UseApplyDimmingV2Props): UseApplyDimmingV2Result {
  ...
  function belongsToSelection(blockish: Blockish | null, selection: Selection): boolean {
    ...
    // CRITICAL FIX: Global/foundational nodes (no program_id, no track_id) belong to ALL programs
    if (!blockish.program_id && !blockish.track_id) {
      if (process.env.NODE_ENV === 'development') {
        console.log('[ApplyDimmingV2] FOUNDATIONAL NODE belongs to all:', {
          id: blockish.id,
          selection
        });
      }
  ...
  const dimmedNodes = useMemo(() => {
    ...
    if (mode === 'single') {
      ...
      return nodes.map(node => {
        ...
        const isGhostNode = node.data?.is_empty_year === true;
        if (isGhostNode) {
