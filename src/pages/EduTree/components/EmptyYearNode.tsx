/**
 * EmptyYear Node - Ghost node for skipped academic years
 * Shows deliberate gap when programs skip years (e.g., IT skips Year 3)
 */
...
export const EmptyYearNode: React.FC<NodeProps> = ({ data }) => {
  ...
  const getMessage = () => {
    if (programId === 'bs_it' && year === 3) {
      return 'IT proceeds directly to capstone';
    }
    return 'No required coursework';
  };

  const getSubMessage = () => {
    if (nextYear) {
      return `Continues to Year ${nextYear}`;
    }
    return 'This program skips this year';
  };
