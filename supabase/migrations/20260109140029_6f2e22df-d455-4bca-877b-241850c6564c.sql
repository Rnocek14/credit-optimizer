-- Fix provenance key resolution to use flat string keys (actual format)
-- Fix res_val normalization with BTRIM
-- Ensure provenance_url gate is present
...
BEGIN
  ...
  IF NEW.status = 'active' AND (OLD.status IS NULL OR OLD.status IS DISTINCT FROM 'active') THEN
    ...
    -- GATE 4: Validate residency provenance (flat string keys - actual format)
    IF (NEW.field_provenance ? 'residency_credits') THEN
      res_prov := NEW.field_provenance->'residency_credits';
    ELSIF (NEW.field_provenance ? 'policy_data.residency_credits') THEN
      res_prov := NEW.field_provenance->'policy_data.residency_credits';
    ELSIF (NEW.field_provenance ? 'residency_policy.min_institutional_credits') THEN
      res_prov := NEW.field_provenance->'residency_policy.min_institutional_credits';
    ELSE
    ...
    IF res_source IS NULL OR res_source NOT IN ('ground_truth', 'human_override') THEN
      RAISE EXCEPTION 'Cannot activate: residency_credits provenance must be ground_truth or human_override, got "%"', COALESCE(res_source, 'NULL');
    END IF;
    
    -- GATE 5: Validate max_transfer provenance (flat string keys - actual format)
    IF (NEW.field_provenance ? 'max_transfer_credits') THEN
      max_prov := NEW.field_provenance->'max_transfer_credits';
    ELSIF (NEW.field_provenance ? 'policy_data.max_transfer_credits') THEN
      max_prov := NEW.field_provenance->'policy_data.max_transfer_credits';
    ELSIF (NEW.field_provenance ? 'transfer_credit_limits.max_total_transfer_credits') THEN
      max_prov := NEW.field_provenance->'transfer_credit_limits.max_total_transfer_credits';
    ELSE
    ...
    IF max_source IS NULL OR max_source NOT IN ('ground_truth', 'human_override') THEN
      RAISE EXCEPTION 'Cannot activate: max_transfer_credits provenance must be ground_truth or human_override, got "%"', COALESCE(max_source, 'NULL');
    END IF;
