...
-- ============================================================
-- backfill_inferred_edges_from_rules(p_limit int)
-- Closes the automation loop: rules → inferred edges → (promotion) → verified edges
-- Idempotent: skips existing active inferred edges via unique index
-- ============================================================
...
DECLARE
  ...
  v_ev RECORD;
BEGIN
  -- Find provider→institution pairs in credit_transfer_rules that have NO active inferred edge.
  -- "Active" = verification_status='inferred' AND superseded_by_edge_id IS NULL
  FOR v_gap IN
    SELECT
      ...
      -- Collect distinct evidence URLs for later insertion
      array_agg(DISTINCT r.evidence_url) FILTER (WHERE r.evidence_url IS NOT NULL AND r.evidence_url <> '') AS evidence_urls,
      array_agg(DISTINCT r.evidence_source_type) FILTER (WHERE r.evidence_source_type IS NOT NULL) AS evidence_source_types
    ...
    HAVING NOT EXISTS (
      ...
      WHERE e.from_entity_type = 'provider'
        ...
        AND e.verification_status = 'inferred'
        AND e.superseded_by_edge_id IS NULL
    )
    ORDER BY COUNT(*) DESC  -- Prioritise providers with most rules
    LIMIT p_limit
  LOOP
    -- Compute confidence: ratio of evidence-backed rules, clamped [0.50, 0.95]
    ...
    v_confidence := GREATEST(0.50,
                      LEAST(0.95,
                        (v_rules_with_evidence::numeric / v_rules_total::numeric)
                      ));

    -- Insert the inferred edge (ON CONFLICT DO NOTHING for idempotency)
    BEGIN
      INSERT INTO institution_transfer_edges (
        ...
        'inferred',
        v_confidence,
        'auto:backfill-from-rules'
      )
      RETURNING id INTO v_new_edge_id;

      v_inserted_count := v_inserted_count + 1;

      -- Attach evidence rows from the rules' evidence_urls
      IF v_gap.evidence_urls IS NOT NULL THEN
        FOR v_ev IN
          SELECT unnest(v_gap.evidence_urls) AS url
        LOOP
          BEGIN
    ...
    EXCEPTION WHEN unique_violation THEN
      -- Edge already exists (race condition or index catch), count as skip
...
-- Grant execute to service_role only (called from ops-cron-runner)
...
COMMENT ON FUNCTION public.backfill_inferred_edges_from_rules IS
  'Auto-creates inferred transfer edges from credit_transfer_rules gaps. Idempotent. Called by ops-cron-runner.';
