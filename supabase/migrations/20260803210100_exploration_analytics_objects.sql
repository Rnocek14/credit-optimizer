-- Exploration analytics views + RPCs (moved from docs/exploration-analytics-*.sql
-- so the objects the keep-set calls at runtime — explorationApi.ts → exploration_bucket_split,
-- exploration_funnel, exploration_daily_rollup, get_active_re_rank_weights — are versioned.
-- All statements are create-or-replace; requires the analytics_events table (live project).
