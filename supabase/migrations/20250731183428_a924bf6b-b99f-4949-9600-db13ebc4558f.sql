-- Phase 1: Data Architecture Foundation - Schema Enhancement
...
-- Update edge_type to include all semantic relationships
ALTER TABLE career_graph_edges 
...
CHECK (edge_type IN (
  'requires', 'teaches', 'unlocks', 'next_role', 'pivot_to', 
  'demonstrates_skill', 'validates_skill', 'prerequisite', 
  'substitutes_for', 'leads_to', 'enhances', 'specializes',
  'certifies', 'prepares_for', 'builds_on'
));
...
CREATE TRIGGER update_substitution_groups_updated_at 
  BEFORE UPDATE ON substitution_groups 
  FOR EACH ROW 
  EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_career_progression_paths_updated_at 
  BEFORE UPDATE ON career_progression_paths 
  FOR EACH ROW 
  EXECUTE FUNCTION update_updated_at_column();