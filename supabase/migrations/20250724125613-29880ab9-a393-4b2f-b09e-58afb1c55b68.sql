...
-- Enable RLS on backup table or drop it
DROP TABLE IF EXISTS public.roadmap_steps_backup;

-- Enable RLS on user_xp table if it exists and doesn't have RLS
DO $$ 
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'user_xp') THEN
