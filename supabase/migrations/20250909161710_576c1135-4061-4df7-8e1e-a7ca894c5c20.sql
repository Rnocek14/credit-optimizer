...
ALTER TABLE requirement_blocks ADD COLUMN IF NOT EXISTS parent_block_id UUID REFERENCES requirement_blocks(id);
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'skills') THEN
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'public' AND tablename = 'course_skills') THEN
