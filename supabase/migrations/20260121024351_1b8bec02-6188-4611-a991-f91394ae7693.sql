-- Drop and recreate with fixes: COALESCE for registry_missing, enrichment_activity per provider, attention-first sorting
...
registry AS (
  SELECT provider_code_norm AS provider, canonical_url_mode, 
...
ORDER BY
  CASE 
    WHEN reg.provider IS NULL THEN 0
    WHEN COALESCE(r.resolution_pct, 100) < 90 OR COALESCE(q.queue_failed, 0) > 10 THEN 1
    WHEN COALESCE(r.resolution_pct, 100) < 100 OR COALESCE(q.queue_failed, 0) > 0 OR COALESCE(q.blocked_last_24h, 0) > 0 THEN 2
    ELSE 3
  END,
  COALESCE(q.blocked_last_24h, 0) DESC,
  COALESCE(q.queue_failed, 0) DESC,
  COALESCE(r.provider, q.provider, e.provider, a.provider, t.provider);