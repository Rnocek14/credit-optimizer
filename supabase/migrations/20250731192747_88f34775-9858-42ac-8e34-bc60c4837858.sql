-- Phase 1.9: Final Critical Completions (Fixed)
...
WITH career_progression_mappings AS (
  SELECT 
    ...
    CASE 
      WHEN (j1.title ILIKE '%Junior%' AND j2.title ILIKE '%Senior%') THEN 0.95
  ...
  AND (
    ...
    OR
    -- Frontend to Senior Frontend
    (j1.title = 'Frontend Engineer' AND j2.title ILIKE '%Senior%Frontend%')
    OR
    -- Backend to Senior Backend
    (j1.title = 'Backend Engineer' AND j2.title ILIKE '%Senior%Backend%')
  )
)
INSERT INTO career_graph_edges (
  from_id, to_id, from_type, to_type, edge_type,
...
AND NOT EXISTS (
  SELECT 1 FROM career_graph_edges 
  WHERE from_id = cpm.junior_job_id AND to_id = cpm.senior_job_id AND edge_type = 'leads_to'
);
...
-- Connect projects to relevant skills and jobs using valid edge types
WITH project_skill_mappings AS (
  SELECT 
    p.id as project_id,
    s.id as skill_id
  FROM career_graph_nodes p
  CROSS JOIN career_graph_nodes s
  WHERE p.node_type = 'project' AND s.node_type = 'skill'
  AND (
...
SELECT 
  ...
  0.85,
  0,
  'Project requires and validates skill proficiency'
FROM project_skill_mappings psm
WHERE NOT EXISTS (
  SELECT 1 FROM career_graph_edges 
  WHERE from_id = psm.project_id AND to_id = psm.skill_id AND edge_type = 'requires'
);