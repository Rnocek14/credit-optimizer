-- Final correctness fixes for plan_invariant_checks and requirement_eligibility
...
BEGIN
  ...
  IF v_prog IS NULL THEN
    ...
    SELECT 
      ...
      null::text as institution_code,  -- Don't hardcode WGU
  ...
  IF v_prog IS NULL THEN
    ...
    -- Use defaults with NULL institution (no hardcoded WGU)
    v_prog := ROW(null::uuid, v_plan.program_id, null::text, 120, 120, 30, 30, 90, 0);
  END IF;
  ...
  IF v_prog.institution_code IS NOT NULL THEN
    ...
    -- Warn if institution lookup fails
...
-- Fix requirement_eligibility with case-proof transfer status and no hardcoded institution
...
BEGIN
  ...
  IF v_prog IS NULL THEN
    -- Fallback to just version table - NO hardcoded WGU
    SELECT 
      null::uuid as id,
      prv.program_id as program_slug,
      null::text as institution_code,  -- NULL means transfer rules won't match
      COALESCE(prv.alt_credit_cap, 30) as alt_credit_cap,
      COALESCE(prv.transfer_max, 90) as transfer_max
    INTO v_prog
    FROM program_requirements_versions prv
  ...
  IF v_prog IS NULL THEN
    -- Use defaults with NULL institution
    v_prog := ROW(null::uuid, v_plan.program_id, null::text, 30::numeric, 90::numeric);
  END IF;
  ...
  -- Join transfer_rules_resolved only if institution_code is known
  -- Use LOWER() for case-proof comparison
  FOR v_course IN
    SELECT DISTINCT
      ec.id AS course_id,
      ec.title,
      ...
      LOWER(tr.acceptance_status) AS transfer_status,  -- Normalize to lowercase
    ...
    -- Transfer rules join: only if we have institution context
    LEFT JOIN transfer_rules_resolved tr 
      ON v_prog.institution_code IS NOT NULL  -- Skip join if no institution
      ...
      AND LOWER(tr.status) = 'active'  -- Case-proof
  ...
  LOOP
    ...
    v_eligibility := jsonb_build_object(
      'transfer_accepted', COALESCE(v_course.transfer_status = 'accepted', false),  -- lowercase
    ...
    ELSIF v_course.transfer_status = 'accepted' THEN  -- lowercase
    ...
    -- Calculate composite score (0-100) with explicit type safety
