-- Fix queue_by_institution - aggregate directly by institution + status
...
BEGIN
  ...
  FROM (
    SELECT 
      pc.institution_code,
      jsonb_object_agg(tgq.status, tgq.cnt ORDER BY tgq.status) as status_obj
    FROM (
      SELECT 
        pc.institution_code,
        tgq.status,
    ...
    JOIN program_catalog pc ON pc.institution_code = tgq.institution_code
