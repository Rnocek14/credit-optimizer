-- Fix all correctness issues in plan_invariant_checks and requirement_eligibility

-- Recreate plan_invariant_checks with all fixes
...
BEGIN
  ...
  IF v_prog IS NULL THEN
    ...
    -- Use defaults
    v_prog := ROW(null::uuid, v_plan.program_id, 'WGU', 120, 120, 30, 30, 90, 0);
  END IF;

  -- Get institution UUID for credit limits lookup
  SELECT id INTO v_institution_id
  FROM institutions
  WHERE code = v_prog.institution_code
  LIMIT 1;

  -- 3) Load institution policy (using actual institution_code)
  ...
  -- C5: PROVIDER_LIMIT check (using institution_id for credit_limits, not hardcoded)
  IF v_institution_id IS NOT NULL THEN
    FOR v_req IN
      SELECT 
        pcc.provider_code,
        SUM(pcc.credits) as provider_credits,
        COALESCE(icl.credit_value, 999) as max_allowed
      ...
      HAVING SUM(pcc.credits) > COALESCE(icl.credit_value, 999)
    LOOP
      v_violations := v_violations || jsonb_build_array(jsonb_build_object(
        'severity', 'block',
        'code', 'PROVIDER_LIMIT',
  ...
  -- C6: REQUIREMENT_INCOMPLETE (renamed from BLOCK_INCOMPLETE) — validate program_requirements
  FOR v_req IN
    SELECT
      pr.id,
      pr.name as title,
...
-- Recreate requirement_eligibility with correct transfer rules join
...
BEGIN
  ...
  IF v_prog IS NULL THEN
    -- Fallback to just version table
    SELECT 
      null::uuid as id,
      prv.program_id as program_slug,
      'WGU' as institution_code,
  ...
  IF v_prog IS NULL THEN
    -- Use defaults
    v_prog := ROW(null::uuid, v_plan.program_id, 'WGU', 30, 90);
  END IF;
  ...
  -- Join transfer_rules_resolved using course code + target_institution
  FOR v_course IN
    SELECT DISTINCT
      ec.id AS course_id,
      ec.title,
    ...
    -- Transfer rules join: match by course code and target institution
    LEFT JOIN transfer_rules_resolved tr 
      ON tr.source_course_code = ec.code
