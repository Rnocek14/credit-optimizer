...
-- ============================================================================
-- FINAL FOOTGUN FIX: NULL-safe normalization + policy scope guards
-- ============================================================================
...
WITH base AS (
  SELECT 
    ...
    -- NULL-safe: empty strings become NULL, preventing accidental matches
    NULLIF(upper(btrim(r.source_course_code)), '') AS rule_course_code_norm,
    CASE
...
FROM base b
-- Tier 1: Direct FK resolution (no scope guard - FK is explicit intent)
LEFT JOIN public.source_courses sc_direct ON sc_direct.id = b.source_course_id
-- Tier 2: Alias resolution with policy scope guard
LEFT JOIN public.source_course_aliases sca 
  ON b.effective_rule_scope <> 'policy'
  AND sca.provider_code_norm = b.rule_provider_norm
...
-- Tier 3: Canonical code direct match with policy scope guard
LEFT JOIN public.source_courses sc_canon
  ON b.effective_rule_scope <> 'policy'
  AND sc_canon.provider_code_norm = b.rule_provider_norm
...
COMMENT ON VIEW public.transfer_rules_resolved IS 'Compliance-grade 3-tier resolver with NULL-safe normalization and policy scope guards on alias/canonical joins';
