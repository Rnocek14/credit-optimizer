-- Final 2%: enforce non-null on active rows in immutability block
...
BEGIN
  ...
  -- IMMUTABILITY: Active rows cannot have critical fields modified or cleared
  IF TG_OP = 'UPDATE' AND OLD.status = 'active' AND NEW.status = 'active' THEN
    ...
    -- Integer-safe residency comparison with non-null enforcement
    old_res_txt := NULLIF(BTRIM(OLD.policy_data->>'residency_credits'), '');
    new_res_txt := NULLIF(BTRIM(NEW.policy_data->>'residency_credits'), '');
    
    IF new_res_txt IS NULL THEN
      RAISE EXCEPTION 'Cannot clear residency_credits on active policy pack';
    END IF;
    ...
    new_res_int := new_res_txt::INTEGER;
    ...
    -- Integer-safe max_transfer comparison with non-null enforcement
    ...
    IF new_max_txt IS NULL THEN
      RAISE EXCEPTION 'Cannot clear max_transfer_credits on active policy pack';
    END IF;
    ...
    new_max_int := new_max_txt::INTEGER;
  ...
  -- GATE 3: Validate residency_credits
  ...
  -- GATE 4: Validate max_transfer_credits
  ...
  -- GATE 7: Degree-level residency with NULL guard
