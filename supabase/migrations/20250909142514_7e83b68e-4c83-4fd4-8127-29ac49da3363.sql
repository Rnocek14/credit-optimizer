-- Seed Software Engineering Program Data with conflict handling
-- Partner policies (only insert if not exists)
...
-- Add prerequisites for Software Engineering courses
INSERT INTO public.edu_prereqs (parent_course_id, child_course_id, prereq_type) VALUES
-- Programming sequence
((SELECT id FROM public.edu_courses WHERE code = 'CS-101'), (SELECT id FROM public.edu_courses WHERE code = 'CS-102'), 'prerequisite'),
((SELECT id FROM public.edu_courses WHERE code = 'CS-102'), (SELECT id FROM public.edu_courses WHERE code = 'CS-201'), 'prerequisite'),
-- Math sequence
((SELECT id FROM public.edu_courses WHERE code = 'MATH-110'), (SELECT id FROM public.edu_courses WHERE code = 'MATH-141'), 'prerequisite'),
((SELECT id FROM public.edu_courses WHERE code = 'MATH-141'), (SELECT id FROM public.edu_courses WHERE code = 'MATH-210'), 'prerequisite'),
-- Advanced CS courses requiring Data Structures
((SELECT id FROM public.edu_courses WHERE code = 'CS-201'), (SELECT id FROM public.edu_courses WHERE code = 'CS-301'), 'prerequisite'),
((SELECT id FROM public.edu_courses WHERE code = 'CS-201'), (SELECT id FROM public.edu_courses WHERE code = 'CS-305'), 'prerequisite'),
((SELECT id FROM public.edu_courses WHERE code = 'CS-201'), (SELECT id FROM public.edu_courses WHERE code = 'CS-310'), 'prerequisite'),
((SELECT id FROM public.edu_courses WHERE code = 'CS-201'), (SELECT id FROM public.edu_courses WHERE code = 'CS-315'), 'prerequisite'),
-- Web Dev requiring Programming II
((SELECT id FROM public.edu_courses WHERE code = 'CS-102'), (SELECT id FROM public.edu_courses WHERE code = 'CS-320'), 'prerequisite'),
-- Capstone requiring multiple courses
((SELECT id FROM public.edu_courses WHERE code = 'CS-310'), (SELECT id FROM public.edu_courses WHERE code = 'CS-499'), 'prerequisite'),
((SELECT id FROM public.edu_courses WHERE code = 'CS-410'), (SELECT id FROM public.edu_courses WHERE code = 'CS-499'), 'prerequisite');
...
-- Add 10 key alternative credit options (CLEP, Sophia, Study.com equivalencies)
...
((SELECT id FROM public.edu_requirements WHERE title = 'English Composition I'), 'CLEP', 'CLEP', 'College Composition', 3, 100, 40, 'Pass score 50'),
((SELECT id FROM public.edu_requirements WHERE title = 'College Algebra'), 'CLEP', 'CLEP', 'College Algebra', 3, 100, 30, 'Pass score 50'),
((SELECT id FROM public.edu_requirements WHERE title = 'Statistics'), 'CLEP', 'CLEP', 'Introductory Business Law', 3, 100, 35, 'Alternative statistics exam'),
((SELECT id FROM public.edu_requirements WHERE title = 'Social Science Elective'), 'CLEP', 'CLEP', 'Introductory Psychology', 3, 100, 25, 'Pass score 50'),
((SELECT id FROM public.edu_requirements WHERE title = 'Humanities Elective'), 'CLEP', 'CLEP', 'Humanities', 3, 100, 40, 'Pass score 50'),

-- Sophia.org options
((SELECT id FROM public.edu_requirements WHERE title = 'English Composition II'), 'ACE', 'Sophia', 'English Composition II', 3, 99, 60, 'Monthly subscription, self-paced'),
((SELECT id FROM public.edu_requirements WHERE title = 'Statistics'), 'ACE', 'Sophia', 'Introduction to Statistics', 3, 99, 50, 'Monthly subscription, self-paced'),
((SELECT id FROM public.edu_requirements WHERE title = 'Communications/Speech'), 'ACE', 'Sophia', 'Public Speaking', 3, 99, 40, 'Monthly subscription, self-paced'),

-- Study.com options
((SELECT id FROM public.edu_requirements WHERE title = 'Calculus I'), 'ACE', 'Study.com', 'Calculus I', 4, 200, 80, 'Proctored final exam required'),
((SELECT id FROM public.edu_requirements WHERE title = 'Programming Fundamentals I'), 'ACE', 'Study.com', 'Computer Science 101', 3, 200, 100, 'Includes programming exercises');