...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const c of allCanonicals ?? []) {
      ...
      canonicalUrlMap.set(c.canonical_code_norm, c.canonical_url ?? "https://clep.collegeboard.org/");
    }
    ...
    const aliasRows = aliasMappings.map((m) => {
      ...
      const evidenceUrl = canonicalUrlMap.get(m.canonical_code.toUpperCase()) ?? "https://clep.collegeboard.org/";
      if (!sourceId) {
