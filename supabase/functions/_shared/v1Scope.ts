...
const FALLBACK_V1_INSTITUTIONS: string[] = [
  // Operational scope (NOT the public display gate). Mirrors institution_v1_scope DB rows
  // as of 2026-05-20 so a DB outage doesn't lock in-flight pipelines out of scope.
  // Display gating for /compare lives in src/lib/planScoring/config.ts VERIFIED_SCHOOL_CODES.
  // Only edit this list when the DB row set changes; keep them in sync.
  'TESU', 'COSC', 'WGU', 'EXCELSIOR', 'EMPIRE',
  'LIBERTY', 'SNHU', 'UMGC', 'GCU', 'ASUO',
];
