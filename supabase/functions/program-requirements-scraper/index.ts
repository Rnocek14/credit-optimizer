...
const NAMESPACE_EDU = '6ba7b810-9dad-11d1-80b4-00c04fd430c8';
...
/**
 * FIX #7: Robust level_year mapping.
 * Handles 0 (placeholder), 500+ (grad), and non-standard numbering.
 */
function courseLevelToYear(level: number): number {
  if (level <= 0) return 1;        // placeholders default to year 1
  if (level < 200) return 1;
  if (level < 300) return 2;
  if (level < 400) return 3;
  return 4;                         // 400+ (including grad 500+) → year 4
}
...
function validateExtraction(
  ...
  for (const block of result.blocks) {
    for (const course of block.courses) {
      if (!course.is_placeholder && (course.level < 100 || course.level > 599)) {
        warnings.push({
...
async function extractRequirements(
  ...
  programName: string,
  openaiKey: string
): Promise<{ result: ExtractionResult; is_truncated: boolean; prompt_chars_used: number }> {
...
- If credit count is missing for a course, default to 3`;
  ...
  const promptContent = markdown.slice(0, MAX_PROMPT_CHARS);
  const is_truncated = markdown.length > MAX_PROMPT_CHARS;
  const prompt_chars_used = promptContent.length;

  const userPrompt = `Extract the degree requirements for "${programName}" at ${institutionCode} from this catalog page${is_truncated ? ' (NOTE: content was truncated, extract what is available)' : ''}:\n\n${promptContent}`;
  ...
  return {
    result: JSON.parse(toolCall.function.arguments) as ExtractionResult,
    is_truncated,
    prompt_chars_used,
  };
}
...
// ── DB Write ────────────────────────────────────────────────────
// FIX #1: Use programCatalogId (UUID) as program_id, not slug
// FIX #2: Upsert on natural keys, not 'id'
// FIX #3: Removed ignoreDuplicates (not a valid Supabase JS option)

// deno-lint-ignore no-explicit-any
async function writeRequirements(
  supabase: any,
  programCatalogId: string,  // UUID from program_catalog.id — used as program_id FK
  institutionCode: string,
  catalogUrl: string,
  ...
  for (const block of extraction.blocks) {
    // Deterministic block ID via UUIDv5 using the program UUID (stable)
    const blockId = await uuidv5(`block:${programCatalogId}:${block.slug}`);

    // FIX #1: program_id = programCatalogId (the real UUID), not a slug
    // FIX #2: conflict on (program_id, slug) via the unique index uq_requirement_blocks_program_slug
    const { error: blockError } = await supabase
      ...
      .upsert({
        ...
        program_id: programCatalogId,
        is_virtual: false,
        hidden: false,
      }, { onConflict: 'id' });
      // Note: We use onConflict 'id' here because our UUIDv5 deterministic IDs
      // ARE the natural key mechanism. The unique index (program_id, slug) serves
      // as a safety net against non-deterministic inserts from other code paths.
    ...
    // FIX #2: conflict on natural key (program_id, requirement_block_id)
    const reqId = await uuidv5(`req:${programCatalogId}:${block.slug}`);
    const { error: reqError } = await supabase
      ...
      }, { onConflict: 'program_id,requirement_block_id' });

    if (reqError) {
      console.error(`Requirement upsert error for ${block.slug}:`, reqError.message);
    }
    ...
    for (const course of block.courses) {
      ...
      // FIX #2: conflict on natural key (institution_code, code_norm)
      // FIX #3: removed ignoreDuplicates (not valid in Supabase JS)
      // FIX #7: use courseLevelToYear() for robust mapping
      const { error: courseError } = await supabase
        ...
        .upsert({
          ...
          level_year: courseLevelToYear(course.level),
          area: block.area,
        ...
        }, { onConflict: 'institution_code,code_norm' });
      ...
      // Link block_member
      // FIX #2: conflict on natural key (block_id, course_id)
      ...
      const { error: memberError } = await supabase
        ...
        .upsert({
          ...
          block_id: blockId,
          course_id: courseId,
        }, { onConflict: 'block_id,course_id' });

      if (memberError) {
        console.error(`Block member upsert error for ${course.code} in ${block.slug}:`, memberError.message);
      }
    }
  }
...
// ── Auth ─────────────────────────────────────────────────────────
// FIX #4: Removed isServiceRole header detection (unsafe pattern)
// FIX #5: Admin JWT uses anon key only, no service-role fallback

interface AuthResult {
  authorized: boolean;
  method: 'cron_secret' | 'admin_jwt' | null;
  userId?: string;
  error?: string;
}
...
async function authenticate(req: Request): Promise<AuthResult> {
  ...
  // Path 2: Admin JWT via getUser
  const authHeader = req.headers.get('Authorization') || '';
  const token = authHeader.replace('Bearer ', '');
  if (!token || token === authHeader) {
    return { authorized: false, method: null, error: 'No valid Authorization header or x-cron-secret' };
  }

  // FIX #5: Use service role key for server-side admin verification
  // (anon key would apply RLS which blocks user_roles reads)
  ...
  if (userError || !userData?.user) {
    return { authorized: false, method: null, error: 'Invalid JWT' };
  }
  ...
  return { authorized: true, method: 'admin_jwt', userId: userData.user.id };
}
...
Deno.serve(async (req) => {
  ...
  // FIX #4: Clean auth — only cron secret OR admin JWT
  ...
  try {
    ...
    const runId = run.id;
    console.log(`[REQ-SCRAPER] Run ${runId} started for ${program.program_slug} @ ${program.institution_code} (auth: ${auth.method})`);
    ...
    // FIX #6: Track truncation
    let extraction: ExtractionResult;
    let is_truncated: boolean;
    let prompt_chars_used: number;
    try {
      ...
      is_truncated = extractionResult.is_truncated;
      prompt_chars_used = extractionResult.prompt_chars_used;
    } catch (extractErr) {
    ...
    // FIX #6: Store extracted JSON + truncation metadata
    await supabase
      .from('requirements_scrape_runs')
      .update({
    ...
    console.log(`[REQ-SCRAPER] Extracted ${extraction.blocks.length} blocks, ${extraction.total_credits} total credits (truncated: ${is_truncated}, chars: ${prompt_chars_used})`);
    ...
    // FIX #1: pass programCatalogId (UUID) as program_id, not slug
    const { coursesWritten, blocksWritten } = await writeRequirements(
      supabase,
      program_catalog_id,    // UUID from program_catalog.id
      program.institution_code,
      program.catalog_url,
      extraction,
      runId
