...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      // Check if ALL migrations already applied (including school scraper)
      const { rows: scraperCheck } = await connection.queryObject(
        `SELECT 1 FROM information_schema.tables
         WHERE table_schema='public' AND table_name='school_scrape_jobs' LIMIT 1`
      );

      const { rows: columnCheck } = await connection.queryObject(
        `SELECT 1 FROM information_schema.columns
         WHERE table_schema='public' AND table_name='credit_transfer_rules'
           AND column_name='source_course_code' LIMIT 1`
      );

      if (columnCheck.length > 0 && scraperCheck.length > 0) {
      ...
      console.log('✅ All migrations complete: 9/9');
      return json(200, { message: "✅ Migrations applied: 9/9 (includes school scraper)" });
