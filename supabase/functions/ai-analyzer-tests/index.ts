...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        { role: "system", content: "Return JSON with: tests[{filename, content}], coverage[], testing_strategy[], mocks_needed[]" },
        { role: "user", content: `Generate ${framework} tests for: ${path}\n\n${fileContent}` }
      ],
    ...
    await supabase.from("ai_analyzer_jobs").insert({
      repo_id: repoId, user_id: user.id, job_type: "tests", status: "completed",
