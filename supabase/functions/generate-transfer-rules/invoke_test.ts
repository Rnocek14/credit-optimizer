...
SOC-1010 Intro to Sociology | Sociology 101: Intro to Sociology | accepted
...
Deno.test("invoke generate-transfer-rules for TESU", async () => {
  // Build auth headers - try cron secret first, then service role key
  const headers: Record<string, string> = { 'Content-Type': 'application/json' };
  
  if (CRON_SECRET) {
    headers['x-cron-secret'] = CRON_SECRET;
    console.log("Using CRON_SECRET auth");
  } else if (SERVICE_ROLE_KEY) {
    headers['Authorization'] = `Bearer ${SERVICE_ROLE_KEY}`;
    console.log("Using SERVICE_ROLE_KEY auth");
  } else {
    // List all env vars that contain 'SECRET' or 'KEY' (names only)
    console.log("Available env vars:");
    // @ts-ignore - Deno env iteration
    for (const [k] of Object.entries(Deno.env.toObject())) {
      if (k.includes('SECRET') || k.includes('KEY') || k.includes('SUPABASE')) {
        console.log(`  ${k} = [set, length=${Deno.env.get(k)?.length}]`);
      }
    }
  ...
  console.log(`Duplicates: ${data.duplicate_count || 0}`);
  
  if (data.candidates) {
    console.log(`\nFirst 15 candidates:`);
    for (const c of data.candidates.slice(0, 15)) {
